...
print_color "blue" "Checking for SSH host keys..."
if [ ! -f /etc/ssh/ssh_host_ed25519_key ]; then
    print_color "yellow" "SSH host keys not found. Generating new host keys..."
    ssh-keygen -t ed25519 -f /etc/ssh/ssh_host_ed25519_key -N ""
    ssh-keygen -t rsa -b 4096 -f /etc/ssh/ssh_host_rsa_key -N ""
    print_color "green" "SSH host keys generated successfully."
else
    print_color "green" "SSH host keys are already present."
fi
